...
{
    LOG_TRACE(
        "burden::Demo3ReqHandler::OnMsg, " << conn_guid << ", " << msg_head << ", msg body len: " << msg_body_len);
    ...
    demo_3_rsp.mutable_err_ctx()->set_err_code(0);

    if (!demo_3_req.udp())
    {
        if (SendToTcpClient(logic_ctx_->scheduler, conn_guid, rsp_msg_head, &demo_3_rsp) != 0)
        {
    ...
    {
        if (SendToUdpClient(logic_ctx_->scheduler, conn_guid, rsp_msg_head, &demo_3_rsp) != 0)
        {
            LOG_ERROR("failed to send to " << conn_guid);
            return;
        }
...
{
    LOG_TRACE("burden::Demo3ReqHandler::OnConnected, trans id: " << trans_id << ", " << peer);
}

void Demo3ReqHandler::OnClosed(TransId trans_id, const Peer& peer, const void* async_data, size_t async_data_len)
{
    LOG_TRACE("burden::Demo3ReqHandler::OnClosed, trans id: " << trans_id << ", " << peer);
}
...
{
    LOG_TRACE("burden::Demo3ReqHandler::OnRecvRsp, trans id: " << trans_id << ", " << peer);
}
...
{
    LOG_TRACE("burden::Demo3ReqHandler::OnTimeout, trans id: " << trans_id << ", " << peer);
}
