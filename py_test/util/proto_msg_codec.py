...
def decode(total_msg_buf, total_msg_len, do_checksum):
    ...
    """
    try:
        checksum_peer, = struct.unpack('!I', total_msg_buf[0:CHECKSUM_FIELD_LEN])
        checksum_self = crc32(total_msg_buf[CHECKSUM_FIELD_LEN:])

        if do_checksum and checksum_self != checksum_peer:
            LOG_ERROR('checksum not match, self checksum: %u, peer checksum: %u' % (checksum_self, checksum_peer))
            return -1, None, None

        msg_head = MsgHead()
        msg_head.passback, msg_head.msg_id = struct.unpack('!ii',
                                                           total_msg_buf[CHECKSUM_FIELD_LEN:MIN_TOTAL_MSG_LEN])

        LOG_DEBUG('total msg len: %d, checksum: %u, passback: %d, msg_id: %d'
